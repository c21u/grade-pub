...
export const isSectionGradable = async (sectionId) => {
  if (!sectionId) {
    return false;
  }
  const [termId, crn] = sectionId.split("/");
  const courseInfo = await buzzapi.post(
    "central.academics.course_catalog.course_info_summary",
    "search",
    { term_code: termId, crn },
  );
  return courseInfo[0].section.ssbsect_gradable_ind === "Y";
};
